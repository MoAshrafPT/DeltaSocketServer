...
io.on("connection", (socket) => {
  socket.on("get-document", async (documentId, userId) => {
    ...
    socket.on("send-changes", (delta, clientVersion, acknowledgeID) => {
      ...
      socket.broadcast
        .to(documentId)
        .emit("receive-changes", delta, tempV, acknowledgeID); // QUESTION: should we increment the server version before or after broadcasting the changes?
    ...
    if ("insert" in delta.ops[1]) {
      if ("insert" in operations[i].ops[1]) {
        if (delta.ops[0].retain >= operations[i].ops[0].retain) {
          ...
          console.log(
            operations[i].ops[1].insert.length,
            "Retain2 in insert insert"
          );
          console.log(
